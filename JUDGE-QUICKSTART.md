...
<!-- Fill in once the deployer key is funded and scripts/deploy.ts has been run against Coston2. -->
Not yet deployed to Coston2 at time of writing — see the "Honest limitations" section of `README.md`. `scripts/deploy.ts` is written and dry-run tested against a local chain; it's one command away once funded.
